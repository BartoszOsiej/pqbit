...
**Next:** fork choice (longest-chain reorg — two independent `--keep-mining` nodes currently fork, and the network correctly rejects blocks that don't chain), explorer, whitepaper. Found a real bug with us: the first coinbase design collided txids across blocks (height lived in the uncommitted signature field) — caught by the double-spend test, fixed by committing height in the prevout.
